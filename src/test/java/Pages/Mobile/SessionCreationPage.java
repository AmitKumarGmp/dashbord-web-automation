...
public class SessionCreationPage {
	...
	public void GotoMyActiveSessions() {
		try {
			...
			action.press(PointOption.point(505, 1870)).waitAction(WaitOptions.waitOptions(Duration.ofMillis(1000)))
					.moveTo(PointOption.point(528, 784)).release().perform();
			Thread.sleep(3000);
	...
	public void GotoMyExpiredSessions() {
		try {
			...
			if (AutomationConfiguration.Tenant.equalsIgnoreCase("GMP")) {
				CommonUtility.GenericMethods.explicitWaitForWebElementOnly(driver, GMPMySessionsGotoExpiredSessions,
						30);
	...
	public void dialerMovement(String Country, ParkingMapper parkingMapper) {
		try {
			...
			if (AutomationConfiguration.Environment.equalsIgnoreCase("Production")
					&& AutomationConfiguration.Tenant.equalsIgnoreCase("Apcoa")) {
				action.press(PointOption.point(553, 1427)).waitAction(WaitOptions.waitOptions(Duration.ofMillis(200)))
						.moveTo(PointOption.point(602, 1444)).release().perform();
			} else {
				if (Country.equalsIgnoreCase("Sweden")) {
					dailerRotationControl(25);
					// action.press(PointOption.point(361,
					// 891)).waitAction(WaitOptions.waitOptions(Duration.ofMillis(1000))).moveTo(PointOption.point(520,
					// 1165)).release().perform();
				} else if (Country.equalsIgnoreCase("Austria")) {
					dailerRotationControl(25);
					// action.press(PointOption.point(553,
					// 1427)).waitAction(WaitOptions.waitOptions(Duration.ofMillis(1000))).moveTo(PointOption.point(847,
					// 1640)).release().perform();
				} else if (Country.equalsIgnoreCase("Poland")) {
					// action.press(PointOption.point(538,
					// 1173)).waitAction(WaitOptions.waitOptions(Duration.ofMillis(1000))).moveTo(PointOption.point(689,
					// 1653)).release().perform(); //emulator
					dailerRotationControl(25);
					// action.press(PointOption.point(359,
					// 891)).waitAction(WaitOptions.waitOptions(Duration.ofMillis(1000))).moveTo(PointOption.point(532,
					// 1136)).release().perform();
	...
	public void StopSession() {
		try {
			...
			Thread.sleep(8000);
			((AndroidDriver<WebElement>) AutomationConfiguration.AppiumDriver)
	...
	public void DailerMovement_AutoCheckout(int x, int y, int a, int b) {
		try {
			...
			action.press(PointOption.point(x, y)).waitAction(WaitOptions.waitOptions(Duration.ofMillis(1000)))
					.moveTo(PointOption.point(a, b)).release().perform();
			// action.press(PointOption.point(538,
			// 1169)).waitAction(WaitOptions.waitOptions(Duration.ofMillis(1000))).moveTo(PointOption.point(612,
			// 1181)).release().perform();
			CommonUtility.GenericMethods.explicitWaitForWebElement(driver, clickOnDialer, 100);
			payAmount.click();
			ActualPaymentSuccess = paySuccessMsg.getText();
			closePayment.click();
	...
	public void dailerRotationControl(int perc) throws InterruptedException {
		if (perc == 25) {
			action.press(PointOption.point(553, 1427)).waitAction(WaitOptions.waitOptions(Duration.ofMillis(1000)))
					.moveTo(PointOption.point(849, 1646)).release().perform();
			Thread.sleep(2000);
		} else if (perc == 50) {
			dailerRotationControl(25);
			action.press(PointOption.point(849, 1646)).waitAction(WaitOptions.waitOptions(Duration.ofMillis(1000)))
					.moveTo(PointOption.point(545, 1968)).release().perform();
			Thread.sleep(2000);
		} else if (perc == 75) {
			dailerRotationControl(50);
			action.press(PointOption.point(545, 1968)).waitAction(WaitOptions.waitOptions(Duration.ofMillis(1000)))
					.moveTo(PointOption.point(206, 1643)).release().perform();
			Thread.sleep(2000);
		} else if (perc == 100) {
			dailerRotationControl(75);
			action.press(PointOption.point(206, 1643)).waitAction(WaitOptions.waitOptions(Duration.ofMillis(1000)))
					.moveTo(PointOption.point(528, 1314)).release().perform();
			Thread.sleep(2000);
		}
	}
}